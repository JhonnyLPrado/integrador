...
@section('title', 'Análisis de Regresión - Delitos La Paz')
        ...
        <div class="row mb-4">
            <div class="col-lg-3 col-6">
                <div class="small-box bg-info">
                    <div class="inner">
                        <h3>{{ number_format($data['estadisticas']['total_delitos']) }}</h3>
            ...
            <div class="col-lg-3 col-6">
                <div class="small-box bg-success">
                    <div class="inner">
                        <h3>{{ $data['estadisticas']['promedio_edad'] }}</h3>
                        <p>Edad Promedio</p>
                    </div>
                    <div class="icon">
            ...
            <div class="col-lg-3 col-6">
                <div class="small-box bg-warning">
                    <div class="inner">
                        <h3>{{ $data['estadisticas']['total_zonas'] }}</h3>
                        <p>Zonas Afectadas</p>
                    </div>
                    <div class="icon">
            ...
            <div class="col-lg-3 col-6">
                <div class="small-box bg-danger">
                    <div class="inner">
                        <h3>{{ $data['estadisticas']['porcentaje_reincidencia'] }}%</h3>
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                <tbody>
                                    @foreach($data['delitos_por_zona'] as $zona => $cantidad)
                                        @php
...
@section('js')
    ...
    <script>
        @if(!isset($error))
            ...
            const delitosPorZona = @json($data['delitos_por_zona']);
            const delitosPorTipo = @json($data['delitos_por_tipo']);
            const delitosPorMes = @json($data['delitos_por_mes']);
            const edadPorSexo = @json($data['edad_por_sexo']);
            const reincidenciaData = @json($data['reincidencia_datos']);
            const violenciaEdadData = @json($data['violencia_por_edad']);
        ...
        function actualizarDatos() {
            fetch('{{ route("api.analisis-regresion.datos") }}')
            ...
            fetch('{{ route("api.analisis-regresion.exportar-pdf") }}', {
                method: 'POST',
                headers: {
        ...
        function exportarExcel() {
            ...
            fetch('{{ route("api.analisis-regresion.exportar-excel") }}', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': '{{ csrf_token() }}'
        ...
        function mostrarPrediccion() {
            Swal.fire({
                ...
                preConfirm: () => {
                    ...
                    return fetch('{{ route("api.analisis-regresion.prediccion") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': '{{ csrf_token() }}'

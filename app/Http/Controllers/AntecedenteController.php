...
class AntecedenteController extends Controller
{
    ...
    {
        $query = Registro::query();        // Si se proporciona un CI específico para ver todos sus registros
        if ($request->filled('ci') && $request->has('ver_todos')) {
            $registros = $query->where('ci', $request->ci)
                              ->orderByDesc('created_at')
                              ->paginate(10);
            
            $persona = $registros->first(); // Para obtener datos de la persona
            
            return view('antecedentes.todos', compact('registros', 'persona'));
        }

        // Búsqueda por CI (búsqueda parcial)
        ...
        // Agrupar por CI y contar antecedentes, usando la fecha más reciente para ordenar
        $registros = $query
            ->selectRaw('ci, nombres, apellido_paterno, apellido_materno, COUNT(*) as antecedentes_count, MAX(created_at) as last_created')
            ->groupBy('ci', 'nombres', 'apellido_paterno', 'apellido_materno')

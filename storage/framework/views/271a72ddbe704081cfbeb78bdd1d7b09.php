...
<?php $__env->startSection('title', 'Firebase Realtime Database - Mapa de Ubicaciones'); ?>
...
// Función para obtener la última ubicación basada en fecha (tolerante a datos sin fecha)
...
const locationDate = new Date(fecha);
    ...
    // Si ninguna tiene fecha válida, mostrar la primera ubicación disponible
    if (anyKey) {
        return { [anyKey]: locations[anyKey] };
    }
    return null;
}
...
function updateLocationsList(locations) {
    ...
    if (showOnlyLastLocation) {
        ...
        // Si ninguna tiene fecha válida, mostrar todas las que existan
        const validKeys = Object.keys(locations).filter(key => {
            const loc = locations[key];
            return loc.fecha_registrada && !isNaN(new Date(loc.fecha_registrada).getTime());
        });
        if (validKeys.length > 0) {
            locationsToShow = {};
            validKeys.forEach(key => locationsToShow[key] = locations[key]);
        } else {
            locationsToShow = locations;
        }
    }
    if (!locationsToShow || Object.keys(locationsToShow).length === 0) {
        listContainer.innerHTML = '<div class="list-group-item text-muted locations-empty"><i class="fas fa-map-pin"></i><br>No hay ubicaciones</div>';
        return;
    }
    Object.keys(locationsToShow).forEach(key => {
        ...
        const isLast = !showOnlyLastLocation && isLastLocationByDate(location, locationsToShow);
...
<?php echo $__env->make('adminlte::page', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\integrador\resources\views/firebase/realtime-map.blade.php ENDPATH**/ ?>